...
Untrusted input is validated by [`src/library/shareValidate.ts`](src/library/shareValidate.ts), which wraps the canonical [`importModelJson`](src/library/jsonIO.ts). No `eval`, no HTML from names (escaped on the share page).
