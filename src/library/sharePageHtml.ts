/**
 * C6 — Server-rendered share page HTML (OG meta + preview shell).
 */
import type { CreatureDesign } from '../creature/types';
import type { ModelExport } from './jsonIO';
import { summarizeShareModel, type ShareSummary } from './shareValidate';
...
function previewDesign(design: CreatureDesign): unknown {
  return {
    name: design.name,
    ...
    muscles: design.muscles.map((m) => ({
      id: m.id,
      startBoneId: m.startBoneId,
      endBoneId: m.endBoneId,
    })),
  };
}

export type SharePageState =
  | { kind: 'ok'; summary: ShareSummary; preview: unknown }
  | { kind: 'not_found' }
  | { kind: 'invalid' }
  | { kind: 'unsupported_version' };

export function sharePageStateFromModel(
  id: string,
  model: ModelExport,
): SharePageState {
  return {
    kind: 'ok',
    summary: summarizeShareModel(id, model),
    preview: previewDesign(model.design),
  };
}
...
<head>
  ...
  <link href="https://fonts.googleapis.com/css2?family=DM+Sans:opsz,wght@0,9..40,400;0,9..40,500;0,9..40,650;1,9..40,400&family=Fraunces:opsz,wght@9..144,600;9..144,700&display=swap" rel="stylesheet" />
...
function sharePageClientJs(): string {
  // Compact preview painter + download wiring (no weights shown).
  return `
(function () {
