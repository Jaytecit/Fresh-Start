...
import type { BoxingOwner } from './scoring';

/** Fighter A solid membership bit (joints/bones of owner 0). */
export const BOXING_A_SOLID = 0b1000_0000;
/** Fighter B solid membership bit (joints/bones of owner 1). */
export const BOXING_B_SOLID = 0b1_0000_0000;

const JOINT_BIT = 0b0001;
const BONE_BIT = 0b0010;
const WORLD_BIT = 0b0100;

export function boxingSolidGroups(
  owner: BoxingOwner,
  kind: 'joint' | 'bone',
): number {
  const part = kind === 'joint' ? JOINT_BIT : BONE_BIT;
  const own = owner === 0 ? BOXING_A_SOLID : BOXING_B_SOLID;
  const opponent = owner === 0 ? BOXING_B_SOLID : BOXING_A_SOLID;
  return encodeGroups(part | own, WORLD_BIT | opponent);
}

/**
 * Enable A↔B solid solving on an already spawned Boxing match creature.
 * Skips sensor probe colliders; leaves ordinary spawn paths untouched.
 */
export function enableBoxingOpponentContact(
  creature: SpawnedCreature,
  owner: BoxingOwner,
): void {
  const jointGroups = boxingSolidGroups(owner, 'joint');
  const boneGroups = boxingSolidGroups(owner, 'bone');
